<?php
  ...
  class RtfImage
  {
          ...
          public $format = 'bmp';
          public $width = 0; // in xExt if wmetafile otherwise in px
          public $height = 0; // in yExt if wmetafile otherwise in px
          public $goalWidth = 0; // in twips
          public $goalHeight = 0; // in twips
          public $pcScaleX = 100; // 100%
          public $pcScaleY = 100; // 100%
          public $binarySize = null; // Number of bytes of the binary data
  ...
  class RtfHtml
  {
    ...
    protected function FormatControlWord($word)
    {
        ...
        if($word->word == "plain" || $word->word == "pard"){
        ...
        }elseif($word->word == "bullet"){ $this->Write("•"); // &#149; &#8226;
        }elseif($word->word == "endash"){ $this->Write("–"); // &#150; &#8211;
        }elseif($word->word == "emdash"){ $this->Write("—"); // &#151; &#8212;

            // more special characters:
        }elseif($word->word == "enspace"){ $this->Write(" "); // &#8194;
        }elseif($word->word == "emspace"){ $this->Write("  "); // &#8195;
        ...
        }elseif($word->word == "sub"){

                $this->OpenTag("sub");
                $this->sup = "sub";


        }elseif($word->word == "nosupersub"){
            if($this->sup == "sup") {
                $this->CloseTag("sup");
                $this->output .= "</sup>";
            }elseif($this->sup == "sub"){
            ...
            $this->Opentag("tr");
        }elseif($word->word == "intbl"){
            $this->Opentag("td");
        }elseif($word->word == "trhdr"){
            $this->CloseTag("th");
            $this->Opentag("th");
...
$string = trim($formatter->Format($reader->root));

file_put_contents('some.html', $string);
